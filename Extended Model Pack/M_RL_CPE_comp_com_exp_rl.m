function [Y, Jacobian] = M_RL_CPE_comp_com_exp_rl(f, par, model_options )
% Description 
...
Y = exp(iw.*tau)./((Rs./Rp + Rs./(R + L.*iw + 1./(W.*iw.^alpha)) + 1).*(R + L.*iw + 1./(W.*iw.^alpha)));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(Rp.*W.^2.*iw.*iw.^(2.*alpha).*exp(iw.*tau).*(Rp + Rs))./(Rp + Rs + R.*Rp.*W.*iw.^alpha + R.*Rs.*W.*iw.^alpha + Rp.*Rs.*W.*iw.^alpha + L.*Rp.*W.*iw.*iw.^alpha + L.*Rs.*W.*iw.*iw.^alpha).^2;
			 case 2 
				 Jacobian = -(Rp.*W.^2.*iw.^(2.*alpha).*exp(iw.*tau).*(Rp + Rs))./(Rp + Rs + R.*Rp.*W.*iw.^alpha + R.*Rs.*W.*iw.^alpha + Rp.*Rs.*W.*iw.^alpha + L.*Rp.*W.*iw.*iw.^alpha + L.*Rs.*W.*iw.*iw.^alpha).^2;
			 case 3 
				 Jacobian = (Rs.*exp(iw.*tau))./(Rp.^2.*(Rs./Rp + Rs./(R + L.*iw + 1./(W.*iw.^alpha)) + 1).^2.*(R + L.*iw + 1./(W.*iw.^alpha)));
			 case 4 
				 Jacobian = -(exp(iw.*tau).*(1./Rp + 1./(R + L.*iw + 1./(W.*iw.^alpha))))./((Rs./Rp + Rs./(R + L.*iw + 1./(W.*iw.^alpha)) + 1).^2.*(R + L.*iw + 1./(W.*iw.^alpha)));
			 case 5 
				 Jacobian = (Rp.*iw.^alpha.*exp(iw.*tau).*(Rp + Rs))./(Rp + Rs + R.*Rp.*W.*iw.^alpha + R.*Rs.*W.*iw.^alpha + Rp.*Rs.*W.*iw.^alpha + L.*Rp.*W.*iw.*iw.^alpha + L.*Rs.*W.*iw.*iw.^alpha).^2;
			 case 6 
				 Jacobian = (Rp.*W.*iw.^alpha.*exp(iw.*tau).*log(iw).*(Rp + Rs))./(Rp + Rs + R.*Rp.*W.*iw.^alpha + R.*Rs.*W.*iw.^alpha + Rp.*Rs.*W.*iw.^alpha + L.*Rp.*W.*iw.*iw.^alpha + L.*Rs.*W.*iw.*iw.^alpha).^2;
			 case 7 
				 Jacobian = (iw.*exp(iw.*tau))./((Rs./Rp + Rs./(R + L.*iw + 1./(W.*iw.^alpha)) + 1).*(R + L.*iw + 1./(W.*iw.^alpha)));
		 otherwise
