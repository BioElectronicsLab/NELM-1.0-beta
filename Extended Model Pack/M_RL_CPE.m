function [Y, Jacobian] = M_RL_CPE(f, par, model_options )
% Series RL-CPE circuit, which is used in electrochemical or non-ideal
%interfaces with parasitic inductance studies
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = log(f.*pi.*2i)./(W.*(f.*pi.*2i).^A.*(R + 1./(W.*(f.*pi.*2i).^A) + L.*f.*pi.*2i).^2);
			 case 2 
				 Jacobian = -(f.*pi.*2i)./(R + 1./(W.*(f.*pi.*2i).^A) + L.*f.*pi.*2i).^2;
			 case 3 
				 Jacobian = -1./(R + 1./(W.*(f.*pi.*2i).^A) + L.*f.*pi.*2i).^2;
			 case 4 
				 Jacobian = 1./(W.^2.*(f.*pi.*2i).^A.*(R + 1./(W.*(f.*pi.*2i).^A) + L.*f.*pi.*2i).^2);
		 otherwise
