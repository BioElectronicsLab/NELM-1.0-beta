function [Y, Jacobian] = M_s_Cole_C_leak_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = (iw.*exp(dT.*iw))./((L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1)))).^2.*(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2);
			 case 2 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1)))).^2;
			 case 3 
				 Jacobian = -exp(dT.*iw)./(R.^2.*(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1)))).^2.*(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1));
			 case 4 
				 Jacobian = (exp(dT.*iw).*log(iw.*tau))./(R.*(iw.*tau).^alpha.*(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1)))).^2.*(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
			 case 5 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))));
			 case 6 
				 Jacobian = (alpha.*iw.*exp(dT.*iw))./(R.*(iw.*tau).^(alpha + 1).*(L.*iw + 1./(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1)))).^2.*(C.*iw + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
		 otherwise
