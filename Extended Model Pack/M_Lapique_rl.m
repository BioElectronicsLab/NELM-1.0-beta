function [Y, Jacobian] = M_Lapique_rl(f, par, model_options )
% % Lapique topology scheme with CPE that used in the electrochemical 
% interfaces or pn-junctions with defects, and tissue studying with
% artifacts
...
Y = exp(iw.*tau)./(R + L.*iw + 1./(1./R_leak + W.*iw.^alpha));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(iw.*tau))./(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1)).^2;
			 case 2 
				 Jacobian = -exp(iw.*tau)./(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1)).^2;
			 case 3 
				 Jacobian = -exp(iw.*tau)./((R_leak.*W.*iw.^alpha + 1).^2.*(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1)).^2);
			 case 4 
				 Jacobian = (R_leak.^2.*iw.^alpha.*exp(iw.*tau))./((R_leak.*W.*iw.^alpha + 1).^2.*(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1)).^2);
			 case 5 
				 Jacobian = (R_leak.^2.*W.*iw.^alpha.*exp(iw.*tau).*log(iw))./((R_leak.*W.*iw.^alpha + 1).^2.*(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1)).^2);
			 case 6 
				 Jacobian = (iw.*exp(iw.*tau))./(R + L.*iw + R_leak./(R_leak.*W.*iw.^alpha + 1));
		 otherwise
