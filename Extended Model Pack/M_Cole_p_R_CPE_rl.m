function [Y, Jacobian] = M_Cole_p_R_CPE_rl(f, par, model_options )
% Description 
...
non_negative_pars=[8  6  5  4  3  2  1];
...
if nargin<=1
	...
	Y.Elements_names(6)={'beta'};
	Y.Elements_names(7)={'dT'};
...
Y = exp(dT.*iw)./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./(Rp + 1./(W.*iw.^beta))));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2;
			 case 2 
				 Jacobian = -exp(dT.*iw)./(R.^2.*(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1));
			 case 3 
				 Jacobian = -exp(dT.*iw)./((1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(Rp + 1./(W.*iw.^beta)).^2);
			 case 4 
				 Jacobian = exp(dT.*iw)./(W.^2.*iw.^beta.*(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(Rp + 1./(W.*iw.^beta)).^2);
			 case 5 
				 Jacobian = (exp(dT.*iw).*log(iw.*tau))./(R.*(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(iw.*tau).^alpha.*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
			 case 6 
				 Jacobian = (exp(dT.*iw).*log(iw))./(W.*iw.^beta.*(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(Rp + 1./(W.*iw.^beta)).^2);
			 case 7 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./(Rp + 1./(W.*iw.^beta))));
			 case 8 
				 Jacobian = (alpha.*iw.*exp(dT.*iw))./(R.*(1./(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(iw.*tau).^(alpha + 1).*(1./(Rp + 1./(W.*iw.^beta)) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
		 otherwise
