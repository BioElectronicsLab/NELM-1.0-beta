function [Y, Jacobian] = M_CPE_s_RCPE_p_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(L.*iw + 1./(1./R + W1.*iw.^alpha1) + 1./(W2.*iw.^alpha2));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2;
			 case 2 
				 Jacobian = -exp(dT.*iw)./((R.*W1.*iw.^alpha1 + 1).^2.*(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2);
			 case 3 
				 Jacobian = (R.^2.*iw.^alpha1.*exp(dT.*iw))./((R.*W1.*iw.^alpha1 + 1).^2.*(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2);
			 case 4 
				 Jacobian = exp(dT.*iw)./(W2.^2.*iw.^alpha2.*(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2);
			 case 5 
				 Jacobian = (R.^2.*W1.*iw.^alpha1.*exp(dT.*iw).*log(iw))./((R.*W1.*iw.^alpha1 + 1).^2.*(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2);
			 case 6 
				 Jacobian = (exp(dT.*iw).*log(iw))./(W2.*iw.^alpha2.*(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1)).^2);
			 case 7 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + 1./(W2.*iw.^alpha2) + R./(R.*W1.*iw.^alpha1 + 1));
		 otherwise
