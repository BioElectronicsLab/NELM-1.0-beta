function [Y, Jacobian] = M_Cole_alpha_beta_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta);
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta).^2;
			 case 2 
				 Jacobian = -(exp(dT.*iw).*(1./(iw.*tau).^alpha + 1).^beta)./(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta).^2;
			 case 3 
				 Jacobian = (R.*beta.*exp(dT.*iw).*log(iw.*tau).*(1./(iw.*tau).^alpha + 1).^(beta - 1))./((iw.*tau).^alpha.*(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta).^2);
			 case 4 
				 Jacobian = -(R.*log(1./(iw.*tau).^alpha + 1).*exp(dT.*iw).*(1./(iw.*tau).^alpha + 1).^beta)./(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta).^2;
			 case 5 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta);
			 case 6 
				 Jacobian = (R.*alpha.*beta.*iw.*exp(dT.*iw).*(1./(iw.*tau).^alpha + 1).^(beta - 1))./((iw.*tau).^(alpha + 1).*(L.*iw + R.*(1./(iw.*tau).^alpha + 1).^beta).^2);
		 otherwise
