function [Y, Jacobian] = M_s_Cole_2x_rl(f, par, model_options )
% Description 
...
if nargin<=1
	...
	Y.Elements_names(3)={'alpha1'};
	Y.Elements_names(4)={'alpha2'};
	Y.Elements_names(5)={'dT'};
	Y.Elements_names(6)={'tau1'};
	Y.Elements_names(7)={'tau2'};
...
Y = exp(dT.*iw)./(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2;
			 case 2 
				 Jacobian = -(exp(dT.*iw).*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1))./(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2;
			 case 3 
				 Jacobian = (R.*exp(dT.*iw).*log(iw.*tau1))./((iw.*tau1).^alpha1.*(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2);
			 case 4 
				 Jacobian = (R.*exp(dT.*iw).*log(iw.*tau2))./((iw.*tau2).^alpha2.*(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2);
			 case 5 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1));
			 case 6 
				 Jacobian = (R.*alpha1.*iw.*exp(dT.*iw))./((iw.*tau1).^(alpha1 + 1).*(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2);
			 case 7 
				 Jacobian = (R.*alpha2.*iw.*exp(dT.*iw))./((iw.*tau2).^(alpha2 + 1).*(L.*iw + R.*(1./(iw.*tau1).^alpha1 + 1./(iw.*tau2).^alpha2 + 1)).^2);
		 otherwise
