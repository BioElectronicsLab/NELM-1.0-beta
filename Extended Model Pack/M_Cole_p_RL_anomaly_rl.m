function [Y, Jacobian] = M_Cole_p_RL_anomaly_rl(f, par, model_options )
% Description 
...
if nargin<=1
	...
	Y.Elements_names(2)={'Lp'};
	Y.Elements_names(3)={'R'};
	Y.Elements_names(4)={'Rp'};
...
Y = exp(dT.*iw)./(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw);
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2;
			 case 2 
				 Jacobian = -(iw.*exp(dT.*iw))./((1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(Rp + Lp.*iw).^2);
			 case 3 
				 Jacobian = -exp(dT.*iw)./(R.^2.*(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1));
			 case 4 
				 Jacobian = -exp(dT.*iw)./((1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(Rp + Lp.*iw).^2);
			 case 5 
				 Jacobian = (exp(dT.*iw).*log(iw.*tau))./(R.*(iw.*tau).^alpha.*(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
			 case 6 
				 Jacobian = (iw.*exp(dT.*iw))./(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw);
			 case 7 
				 Jacobian = (alpha.*iw.*exp(dT.*iw))./(R.*(iw.*tau).^(alpha + 1).*(1./(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))) + L.*iw).^2.*(1./(Rp + Lp.*iw) + 1./(R.*(1./(iw.*tau).^alpha + 1))).^2.*(1./(iw.*tau).^alpha + 1).^2);
		 otherwise
