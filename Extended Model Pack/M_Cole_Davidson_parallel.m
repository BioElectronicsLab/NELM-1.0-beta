function [Y, Jacobian] = M_Cole_Davidson_parallel(f, par, model_options )
% Description 
...
if nargin<=1
	Y.version='LEVM_like';
	Y.Elements_names(1)={'R'};
	Y.Elements_names(2)={'gamma'};
	Y.Elements_names(3)={'tau'};
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*tau + 1).^gamma./R.^2;
			 case 2 
				 Jacobian = (log(iw.*tau + 1).*(iw.*tau + 1).^gamma)./R;
			 case 3 
				 Jacobian = (gamma.*iw.*(iw.*tau + 1).^(gamma - 1))./R;
		 otherwise
