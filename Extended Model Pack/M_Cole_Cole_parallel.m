function [Y, Jacobian] = M_Cole_Cole_parallel(f, par, model_options )
% Description 
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -((iw.*tau).^alpha + 1)./R.^2;
			 case 2 
				 Jacobian = (log(iw.*tau).*(iw.*tau).^alpha)./R;
			 case 3 
				 Jacobian = (alpha.*iw.*(iw.*tau).^(alpha - 1))./R;
		 otherwise
