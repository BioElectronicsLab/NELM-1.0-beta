function [Y, Jacobian] = M_2x_R_CPE_Ladder_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2;
			 case 2 
				 Jacobian = -exp(dT.*iw)./(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2;
			 case 3 
				 Jacobian = -exp(dT.*iw)./((R2 + 1./(W2.*iw.^alpha2)).^2.*(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1).^2.*(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2);
			 case 4 
				 Jacobian = (iw.^alpha1.*exp(dT.*iw))./((1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1).^2.*(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2);
			 case 5 
				 Jacobian = exp(dT.*iw)./(W2.^2.*iw.^alpha2.*(R2 + 1./(W2.*iw.^alpha2)).^2.*(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1).^2.*(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2);
			 case 6 
				 Jacobian = (W1.*iw.^alpha1.*exp(dT.*iw).*log(iw))./((1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1).^2.*(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2);
			 case 7 
				 Jacobian = (exp(dT.*iw).*log(iw))./(W2.*iw.^alpha2.*(R2 + 1./(W2.*iw.^alpha2)).^2.*(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1).^2.*(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1)).^2);
			 case 8 
				 Jacobian = (iw.*exp(dT.*iw))./(R1 + L.*iw + 1./(1./(R2 + 1./(W2.*iw.^alpha2)) + W1.*iw.^alpha1));
		 otherwise
