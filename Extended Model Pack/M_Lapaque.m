function [Y, Jacobian] = M_Lapaque(f, par, model_options )
% Lapique topology scheme with CPE that used in the electrochemical 
% interfaces or pn-junctions with defects, and tissue studying 
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = (W.*log(f.*pi.*2i).*(f.*pi.*2i).^A)./((W.*(f.*pi.*2i).^A + 1./R).^2.*(R_1 + 1./(W.*(f.*pi.*2i).^A + 1./R)).^2);
			 case 2 
				 Jacobian = -1./(R.^2.*(W.*(f.*pi.*2i).^A + 1./R).^2.*(R_1 + 1./(W.*(f.*pi.*2i).^A + 1./R)).^2);
			 case 3 
				 Jacobian = -1./(R_1 + 1./(W.*(f.*pi.*2i).^A + 1./R)).^2;
			 case 4 
				 Jacobian = (f.*pi.*2i).^A./((W.*(f.*pi.*2i).^A + 1./R).^2.*(R_1 + 1./(W.*(f.*pi.*2i).^A + 1./R)).^2);
		 otherwise
