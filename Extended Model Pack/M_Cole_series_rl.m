function [Y, Jacobian] = M_Cole_series_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(L.*iw + R.*(1./(iw.*tau).^alpha + 1));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau).^alpha + 1)).^2;
			 case 2 
				 Jacobian = -(exp(dT.*iw).*(1./(iw.*tau).^alpha + 1))./(L.*iw + R.*(1./(iw.*tau).^alpha + 1)).^2;
			 case 3 
				 Jacobian = (R.*exp(dT.*iw).*log(iw.*tau))./((iw.*tau).^alpha.*(L.*iw + R.*(1./(iw.*tau).^alpha + 1)).^2);
			 case 4 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + R.*(1./(iw.*tau).^alpha + 1));
			 case 5 
				 Jacobian = (R.*alpha.*iw.*exp(dT.*iw))./((iw.*tau).^(alpha + 1).*(L.*iw + R.*(1./(iw.*tau).^alpha + 1)).^2);
		 otherwise
