function [Y, Jacobian] = M_Cole_R_leak_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak));
...
if isfield(model_options,'get_analytical_jacobian')
	 if model_options.get_analytical_jacobian
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak)).^2;
			 case 2 
				 Jacobian = -exp(dT.*iw)./(R.^2.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak)).^2.*(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak).^2.*(1./(iw.*tau).^alpha + 1));
			 case 3 
				 Jacobian = -exp(dT.*iw)./(R_leak.^2.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak)).^2.*(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak).^2);
			 case 4 
				 Jacobian = (exp(dT.*iw).*log(iw.*tau))./(R.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak)).^2.*(iw.*tau).^alpha.*(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak).^2.*(1./(iw.*tau).^alpha + 1).^2);
			 case 5 
				 Jacobian = (iw.*exp(dT.*iw))./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak));
			 case 6 
				 Jacobian = (alpha.*iw.*exp(dT.*iw))./(R.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak)).^2.*(iw.*tau).^(alpha + 1).*(1./(R.*(1./(iw.*tau).^alpha + 1)) + 1./R_leak).^2.*(1./(iw.*tau).^alpha + 1).^2);
		 otherwise
