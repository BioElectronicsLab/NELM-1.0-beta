...
for j=1:Ch_Num
    Channels(j).Model=@M_RL_CPE_rl;                                            %Defining the basic model
    Channels(j).Model_Options.fix_pars=[4; 1];                             %Setting the parameters, which will be freezed during approximation.
    Channels(j).Model_Options.pars_to_be_fixed=[4];                        %Setting the parameters, which will be freezed during approximation after warming-up mode.
    Channels(j).x0_init=[15e-3 15e3 1e-9 1e-6];                                 %Setting up the starting point
