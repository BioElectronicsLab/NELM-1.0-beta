function [Y, Jacobian] = M_Cole_Cole_FM_rl(f, par, model_options )
% Description 
...
Y = exp(iw.*tau)./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1));
...
if isfield(model_options,'get_J')
	 if model_options.get_J
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = (exp(iw.*tau).*log(iw.*tau))./(R.*(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1).^2.*(iw.*tau).^A.*(1./(iw.*tau).^A + 1).^2.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1)).^2);
			 case 2 
				 Jacobian = -(iw.*exp(iw.*tau))./(L.*iw + 1./(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1)).^2;
			 case 3 
				 Jacobian = -exp(iw.*tau)./(R.^2.*(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1).^2.*(1./(iw.*tau).^A + 1).*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1)).^2);
			 case 4 
				 Jacobian = -exp(iw.*tau)./(R_1.^2.*(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1).^2.*(L.*iw + 1./(1./(R.*(1./(iw.*tau).^A + 1)) + 1./R_1)).^2);
			 case 5 
				 Jacobian = (exp(iw.*tau).*(R.^2.*R_1.*iw.*tau + A.*R.*R_1.^2.*(iw.*tau).^A + L.*R.^2.*iw.^2.*tau + R.*R_1.^2.*iw.*tau.*(iw.*tau).^A + 2.*R.^2.*R_1.*iw.*tau.*(iw.*tau).^A + 2.*L.*R.^2.*iw.^2.*tau.*(iw.*tau).^A + R.*R_1.^2.*iw.*tau.*(iw.*tau).^(2.*A) + R.^2.*R_1.*iw.*tau.*(iw.*tau).^(2.*A) + L.*R.^2.*iw.^2.*tau.*(iw.*tau).^(2.*A) + L.*R_1.^2.*iw.^2.*tau.*(iw.*tau).^(2.*A) + 2.*L.*R.*R_1.*iw.^2.*tau.*(iw.*tau).^A + 2.*L.*R.*R_1.*iw.^2.*tau.*(iw.*tau).^(2.*A)))./(tau.*(R.*R_1 + L.*R.*iw + R.*R_1.*(iw.*tau).^A + L.*R.*iw.*(iw.*tau).^A + L.*R_1.*iw.*(iw.*tau).^A).^2);
		 otherwise
