function [Y, Jacobian] = M_Lapicque_rl(f, par, model_options )
% Description 
...
Y = exp(dT.*iw)./(Rs + L.*iw + 1./(1./Rp + W.*iw.^alpha));
...
if isfield(model_options,'get_J')
	 if model_options.get_J
 	 ...
 	 function Jacobian=Jacobian_func(n) 
		 switch n
			 case 1 
				 Jacobian = -(iw.*exp(dT.*iw))./(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1)).^2;
			 case 2 
				 Jacobian = -exp(dT.*iw)./((Rp.*W.*iw.^alpha + 1).^2.*(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1)).^2);
			 case 3 
				 Jacobian = -exp(dT.*iw)./(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1)).^2;
			 case 4 
				 Jacobian = (Rp.^2.*iw.^alpha.*exp(dT.*iw))./((Rp.*W.*iw.^alpha + 1).^2.*(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1)).^2);
			 case 5 
				 Jacobian = (Rp.^2.*W.*iw.^alpha.*exp(dT.*iw).*log(iw))./((Rp.*W.*iw.^alpha + 1).^2.*(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1)).^2);
			 case 6 
				 Jacobian = (iw.*exp(dT.*iw))./(Rs + L.*iw + Rp./(Rp.*W.*iw.^alpha + 1));
		 otherwise
